...
// writeCriteriaValues writes the dynamic criteria values section extracted from
// recipe command flag completions.
func writeCriteriaValues(buf *bytes.Buffer, meta *cliMeta) {
	...
	// Collect flags that have completions (criteria fields).
...
func writeWorkflowExamples(buf *bytes.Buffer) {
	...
	fmt.Fprintf(buf, "```bash\n")
	fmt.Fprintf(buf, "aicr verify --bundle ./bundles\n")
	fmt.Fprintf(buf, "```\n\n")
}
